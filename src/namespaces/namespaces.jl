...
function Base.getindex(names::AbstractNamespace, name::Symbol)
    return getname(names, name, nothing)
end
function notify(
...
function unsubscribe!(
    ...
    return dropsubscriptions!(names, observer, fn, vars)
end
subscribe!(
    names::AbstractNamespace,
    observer::AbstractObserver,
...
function dirty!(namespace::ENamespace, name::Symbol, dirt::Bool = true)
    return if dirt && name ∉ namespace.dirty
