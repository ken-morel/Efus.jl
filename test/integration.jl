...
@testset "Integration Tests" begin
    @testset "End-to-End efus Macro" begin
        ...
        Label = (;text, size=12) -> "Label(\"$text\", $size)"
        Button = (;text, onclick=nothing) -> "Button(\"$text\")"
        Container = (;children...) -> "Container([$(join(children, ", "))])"
    ...
    @testset "Reactive Integration" begin
        ...
        # Mock component
        Display = (;count, msg) -> "Display: $msg ($count)"
        ...
        # Initial state
        @test contains(string(component), "0")
        @test contains(string(component), "Hello")
